...
class SkisMeta{
	...
	public function render_box_fields(){
		...
		$ski_title = get_post_meta($post->ID, 'ski_title', true);
		$paypal_embed = get_post_meta($post->ID, 'ski_paypal_embed', true);


		//ski title field
		wp_nonce_field('ski_title_verify', 'ski_title_field_value');

echo <<<EOT
	<label for="ski_title_field">Ski Name</label>
	<input type="text" id="ski_title_field" name="ski_title_field" value="$ski_title" style="width: 100%;">
EOT;
...
EOT;
	...
	public function save($post_id){
		if(!isset($_POST['ski_title_field_value']) || !isset($_POST['ski_paypal_field_value'])){
			return;
		}

		$title_value = $_POST['ski_title_field_value'];
		$paypal_value = $_POST['ski_paypal_field_value'];

		if(!wp_verify_nonce($title_value, 'ski_title_verify') || !wp_verify_nonce($paypal_value, 'ski_paypal_verify')){
			die('couldn\'t verify');
		}
		...
		update_post_meta($post_id, 'ski_title', $_POST['ski_title_field']);
